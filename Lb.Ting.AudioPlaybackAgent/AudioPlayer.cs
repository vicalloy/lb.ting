...
namespace Lb.Ting.AudioPlaybackAgent
{
    public class AudioPlayer : AudioPlayerAgent
    {
        ...
        {
            String s = Utils.ReadFile(Cfg.SONGS_FILENAME);
            if ("".Equals(s))
            {
                return;
            }
            SongDetailListRoot songsRoot = SongDetailListRoot.FromJson(s);
            List<SongDetail> songs = songsRoot.data.songList;
            player.Track = songs[new Random().Next(songs.Count)].toTrack(songsRoot.data.xcode);
            player.Play();
        }
